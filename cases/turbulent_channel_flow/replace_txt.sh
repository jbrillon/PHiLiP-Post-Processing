...
for i in ${!restartIndices[@]}; do
    ...
    replaceStrings=(\
        ...
        "    set output_vorticity_magnitude_field_in_addition_to_velocity = " \
    )

    newStrings=(\
        "  set final_time                                             = ${end_time_string}" \
        "  set restart_files_directory_name                           = restart_files_cPlus_C1_Re5200" \
    ...
    for j in ${!replaceStrings[@]}; do
        ...
        # sed -i -e 's/.*${replaceStrings[$j]}.*/set output_vorticity_magnitude_field_in_addition_to_velocity = true\nset output_viscosity_field_in_addition_to_velocity           = true\nset output_density_field_in_addition_to_velocity             = true\nset output_velocity_number_of_subvisions                     = 2/' ${filename}
    done
