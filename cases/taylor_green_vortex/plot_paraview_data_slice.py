...
def plot_vorticity_plane(path,filename_without_extension,file_extension,fig_directory,fig_prepre_fix,
    ...
    if(plot_reference_result):
        [x_ref,y_ref,w_ref] = get_reference_result()
        plt.contour(x_ref,y_ref,w_ref, levels = contour_levels, colors=('k',),linewidths=(0.5,))
    ...
    if(subdivide):
        ...
        if(plot_reference_result):
            ...
            contour_line_clr_for_result = 'k'
        cs_lines = plt.tricontour(tri_refi,z_test_refi,levels=contour_levels,colors=(contour_line_clr_for_result,),linewidths=(0.5,))
    else:
        if(fill_contour):
    ...
    print('\t ... Saving figure ...')
    # plt.show()
    plt.savefig(fig_directory+"/"+figure_filename+'.'+figure_filetype,format=figure_filetype,dpi=500,bbox_inches=0)#,rasterized=True
    plt.close()
    print('\t     Saved.')
    print("---------------------------------------------")
    return
...
filesystem+"NarvalFiles/2023_JCP/filtered_dns_viscous_tgv/viscous_TGV_ILES_NSFR_cDG_IR_2PF_GL_OI-0_dofs0256_p7_procs1024/solution_files/",\
filesystem+"NarvalFiles/2023_JCP/verification/viscous_TGV_ILES_NSFR_cDG_IR_2PF_GL_OI-0_dofs0256_p3_procs1024/solution_files/",\
filesystem+"NarvalFiles/2023_JCP/verification/viscous_TGV_ILES_NSFR_cDG_IR_2PF-Roe_GL_OI-0_dofs0256_p3_procs1024/solution_files/",\
filesystem+"NarvalFiles/2023_JCP/verification/viscous_TGV_ILES_std_strong_DG_Roe_GL_OI-4_dofs0256_p3_CFL-0.15_procs1024/solution_files/",\
